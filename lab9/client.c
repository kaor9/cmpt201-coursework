...
It is TCP becuase it is using SOCK_STREAM
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? Info is from standard input since were
reading whatevers from there then writing it to the sfd.
4. How does the client program end? How can you tell that in the code?
File ends when the while loop ends, which is when the read is less than 1. This
means either we get an error which returns -1 or we do an EOF.
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
